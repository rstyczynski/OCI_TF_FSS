...
_generated_dir() {
  ...
  root_dir="$(_root_dir)"
  base="${TF_GENERATED_ROOT:-${root_dir}/progress/sprint_16/generated_tf}"
  dir="${base}/${name}"
...
test_IT1_sprint16_resource_manager_advanced_workflow() {
  ...
  jq -n \
    ...
    --arg display_name "fss-sprint16-orm-mt" \
  ...
  (
    ...
    _apply_stack "$mt_stack_id" "fss-sprint16-orm-mount-target-apply" "$mt_apply_json" >"${workdir}/mount_target_apply_job_ocid.txt"
    ...
    jq -n \
      ...
      '{region: $region, compartment_ocid: $compartment_ocid, availability_domain: $availability_domain, existing_mount_target_ocid: $mount_target_ocid, filesystem_display_name: "fss-sprint16-orm-fs", export_1_path: "/data", add_export_2: true, export_2_path: "/logs"}' >"$fs_vars"
    ...
    _apply_stack "$fs_stack_id" "fss-sprint16-orm-filesystem-export-apply" "$fs_apply_json" >"${workdir}/filesystem_export_apply_job_ocid.txt"
    oci resource-manager job get-job-tf-state \
    ...
    _destroy_stack "$fs_stack_id" "fss-sprint16-orm-filesystem-export-destroy" "$fs_destroy_json" >"${workdir}/filesystem_export_destroy_job_ocid.txt"

    mt_destroy_json="${workdir}/mount_target_destroy_job.json"
    _destroy_stack "$mt_stack_id" "fss-sprint16-orm-mount-target-destroy" "$mt_destroy_json" >"${workdir}/mount_target_destroy_job_ocid.txt"
  ...
  if [[ "$ec" -ne 0 && "${SKIP_TEARDOWN:-false}" != "true" ]]; then
    if [[ -n "${fs_stack_id:-}" && "${fs_stack_id}" != "null" ]]; then
      _destroy_stack "$fs_stack_id" "fss-sprint16-orm-filesystem-export-cleanup" "${workdir}/filesystem_export_cleanup_destroy_job.json" || true
      _delete_stack "$fs_stack_id"
    fi
    if [[ -n "${mt_stack_id:-}" && "${mt_stack_id}" != "null" ]]; then
      _destroy_stack "$mt_stack_id" "fss-sprint16-orm-mount-target-cleanup" "${workdir}/mount_target_cleanup_destroy_job.json" || true
