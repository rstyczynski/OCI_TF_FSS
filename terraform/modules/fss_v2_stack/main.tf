...
locals {
  ...
  subnet_availability_domain_raw = try(data.oci_core_subnet.selected.availability_domain, null)
  subnet_availability_domain     = try(length(trimspace(local.subnet_availability_domain_raw)) > 0, false) ? local.subnet_availability_domain_raw : null
  random_ad_name                 = length(random_shuffle.picked_ad) > 0 ? random_shuffle.picked_ad[0].result[0] : null
  effective_availability_domain  = coalesce(var.availability_domain, local.subnet_availability_domain, local.random_ad_name)
