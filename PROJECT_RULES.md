...
Sprints build Terraform products in `terraform/modules/<product>_sprint<N>/` as the versioned artifact. At Phase 5 (Documentor), every sprint that delivers an operator-facing Terraform module MUST create or update a stable symlink in `terraform/modules/` pointing to that sprint's directory (e.g. `terraform/modules/fss_stack -> fss_stack_sprint18`). The stable name carries no sprint suffix and is the only path operators and regression tests may use; sprint-suffixed paths are reserved for sprint acceptance tests and historical audit.

Documentor release step:

```bash
cd terraform/modules
ln -sfn fss_stack_sprint18 fss_stack
ls -la fss_stack
...
Every `sprint_N_design.md` that delivers an operator-facing Terraform module MUST include a `Stable release name:` field before Phase 3 (Construction) begins. The Documentor uses this field at Phase 5 to create the R1 release symlink. Example:
